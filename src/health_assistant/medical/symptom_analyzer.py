...
class SymptomAnalysisRequest(BaseModel):
    ...
    symptoms: List[Dict[str, str]] = Field(..., description="List of symptoms with details")
    ...
    lifestyle_factors: Optional[Dict[str, str]] = Field(None, description="Lifestyle information")
...
class SymptomAnalyzer:
    ...
    def __init__(self):
        ...
        self.emergency_symptoms = {
            ...
            "neurological": ["sudden weakness", "facial drooping", "slurred speech", "severe headache"],
            "bleeding": ["severe bleeding", "uncontrolled bleeding", "massive bleeding"],
            "consciousness": ["unconscious", "loss of consciousness", "fainting"],
            "allergic": ["severe allergic reaction", "anaphylaxis", "swelling of face/throat"],
        ...
        self.symptom_patterns = {
            ...
            "stroke": {
                "symptoms": ["sudden weakness", "facial drooping", "speech problems", "severe headache"],
                "conditions": ["stroke", "transient ischemic attack"],
                "urgency": "emergency"
            },
            "digestive": {
                "symptoms": ["nausea", "vomiting", "diarrhea", "abdominal pain"],
                "conditions": ["gastroenteritis", "food poisoning", "inflammatory bowel disease"],
                "urgency": "moderate"
            }
        }
    
    async def analyze_symptoms(self, request: SymptomAnalysisRequest) -> SymptomAnalysisResult:
        """Perform comprehensive symptom analysis."""
        try:
            logger.info(f"Starting symptom analysis for patient age {request.patient_age}")
            
            # Parse and categorize symptoms
            parsed_symptoms = self._parse_symptoms(request.symptoms)
            
            # Check for emergency conditions
            emergency_detected, red_flags = self._check_emergency_conditions(parsed_symptoms)
            
            # Assess risk level
            risk_level = self._assess_risk_level(parsed_symptoms, request.medical_history)
    ...
    def _categorize_body_system(self, symptom_name: str) -> BodySystem:
        ...
        # Cardiovascular
        if any(word in symptom_name for word in ["chest pain", "heart", "palpitation", "cardiac"]):
            return BodySystem.CARDIOVASCULAR
        
        # Respiratory
        if any(word in symptom_name for word in ["breathing", "cough", "lung", "respiratory"]):
            return BodySystem.RESPIRATORY
        
        # Neurological
        if any(word in symptom_name for word in ["headache", "dizziness", "weakness", "numbness", "seizure"]):
            return BodySystem.NEUROLOGICAL
        
        # Gastrointestinal
        if any(word in symptom_name for word in ["nausea", "vomiting", "diarrhea", "stomach", "abdominal"]):
            return BodySystem.GASTROINTESTINAL
        
        # Musculoskeletal
        if any(word in symptom_name for word in ["pain", "ache", "joint", "muscle", "bone"]):
    ...
    def _check_emergency_conditions(self, symptoms: List[Symptom]) -> Tuple[bool, List[str]]:
            ...
            for category, emergency_terms in self.emergency_symptoms.items():
                for term in emergency_terms:
                    if term in symptom_name:
                        ...
                        red_flags.append(f"🚨 {term.title()} - requires immediate medical attention")
            
            # Check severity levels
            if symptom.severity in [SymptomSeverity.SEVERE, SymptomSeverity.CRITICAL]:
                if symptom.body_system in [BodySystem.CARDIOVASCULAR, BodySystem.NEUROLOGICAL]:
                    emergency_detected = True
                    red_flags.append(f"🚨 Severe {symptom.body_system.value} symptoms detected")
        
        return emergency_detected, red_flags
    
    def _assess_risk_level(self, symptoms: List[Symptom], medical_history: Optional[List[str]]) -> str:
        """Assess overall risk level based on symptoms and history."""
        
        # Count severe symptoms
        severe_count = sum(1 for s in symptoms if s.severity in [SymptomSeverity.SEVERE, SymptomSeverity.CRITICAL])
        
        # Check for high-risk body systems
        high_risk_systems = [BodySystem.CARDIOVASCULAR, BodySystem.NEUROLOGICAL, BodySystem.RESPIRATORY]
        high_risk_symptoms = [s for s in symptoms if s.body_system in high_risk_systems]
        
        # Consider medical history
        high_risk_conditions = ["heart disease", "diabetes", "stroke", "cancer"] if medical_history else []
        has_high_risk_history = any(condition in " ".join(medical_history or []).lower() 
    ...
    def _identify_conditions(self, symptoms: List[Symptom], request: SymptomAnalysisRequest) -> List[Dict[str, float]]:
        ...
        for pattern_name, pattern_data in self.symptom_patterns.items():
            ...
            matches = sum(1 for ps in pattern_symptoms if any(ps in sn for sn in symptom_names))
    ...
    def _generate_recommendations(self, risk_level: str, emergency_detected: bool, 
                                symptoms: List[Symptom], request: SymptomAnalysisRequest) -> List[str]:
        ...
        for symptom in symptoms:
            if symptom.body_system == BodySystem.RESPIRATORY:
                ...
                recommendations.append("🍽️ Consider bland diet (BRAT: bananas, rice, applesauce, toast)")
            elif symptom.body_system == BodySystem.MUSCULOSKELETAL:
                recommendations.append("🧊 Apply ice for acute injuries, heat for muscle tension")
    ...
    def get_symptom_suggestions(self, partial_symptom: str) -> List[str]:
        ...
        common_symptoms = [
            ...
            "dizziness", "weakness", "joint pain", "muscle aches", "rash", "insomnia"
        ]
