...
class HealthConsultationRequest(BaseModel):
    ...
    severity: Optional[str] = Field(None, description="Symptom severity (mild/moderate/severe)")
...
class HealthConsultant:
    ...
    def __init__(self, database: Optional[HealthDatabase] = None):
        ...
        self.emergency_keywords = [
            ...
            "allergic reaction", "severe injury", "poisoning",
    ...
    async def conduct_consultation(self, request: HealthConsultationRequest) -> HealthConsultationResponse:
        ...
        try:
            ...
            recommendations = await self._generate_recommendations(request, profile, risk_level)
            ...
            response = HealthConsultationResponse(
                ...
                follow_up_needed=risk_level in [HealthRiskLevel.HIGH, HealthRiskLevel.CRITICAL],
    ...
    async def _assess_risk_level(self, request: HealthConsultationRequest, 
                                profile: Optional[HealthProfile]) -> HealthRiskLevel:
        ...
        if request.severity == "severe" or (
            profile and any(condition in ["diabetes", "heart disease", "hypertension"] 
    ...
    async def _generate_assessment(self, request: HealthConsultationRequest,
                                 profile: Optional[HealthProfile],
                                 risk_level: HealthRiskLevel) -> str:
    ...
    async def _generate_recommendations(self, request: HealthConsultationRequest,
                                      profile: Optional[HealthProfile],
                                      risk_level: HealthRiskLevel) -> List[str]:
    ...
    def get_consultation_history(self, user_id: str) -> List[HealthConsultationResponse]:
        """Get user's consultation history."""
        self.privacy_manager.log_access(user_id, "history_access", "consultation_history")
